...
class InvertedBottleneck(nn.Module):
    ...
    def __init__(self, c, k=3):
        ...
        self.cv2 = DWConv(c * 2, c * 2, k=k)
        self.cv3 = Conv(c * 2, c, 1, 1)
...
class SCSA(nn.Module):
    ...
    def forward(self, x):
        x_s = self.smsa(x)
        x_c = self.pcsa(x_s)
        return self.proj(x_c)
    
class CSPStage(nn.Module):
    """
    CSPStage module for RepGFPN.
    Integrates RepBottleneck for efficient feature representation and re-parameterization.
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU or SiLU are handled in Conv
        self.m = nn.Sequential(*(RepBottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        """Forward pass through the CSPStage with Rep modules."""
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))