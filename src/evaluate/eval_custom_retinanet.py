...
# Tự động trỏ import về thư mục gốc của project để nạp file model
sys.path.append(str(Path(__file__).resolve().parents[2]))
try:
    # Yêu cầu file custom_retinanet.py phải được đặt trong thư mục src/models/
    from src.models.custom_retinanet import build_model
except ImportError as e:
    raise ImportError(f"❌ Lỗi: Không thể import 'build_model'. Hãy chắc chắn bạn đã để custom_retinanet.py vào src/models/. Chi tiết: {e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Custom RetinaNet")
    ...
    print("🚀 Đang khởi tạo Model Custom RetinaNet (CSPDarknet + PANet + CBAM)...")
    ...
    try:
        ...
        model.to(DEVICE)
        model.eval()
        print(f"✅ Đã nạp thành công trọng số Custom RetinaNet")
    except Exception as e:
        print(f"❌ Lỗi nạp model: {e}")
        exit()

    # Lưu ý: Ở file này bạn từng định nghĩa class T_Compose, nay ta có thể dùng luôn ToTensor ở trên
    ...
    print("\n" + "=" * 65)
    print(f"KẾT QUẢ BENCHMARK: Custom RetinaNet")
    print("=" * 65)
